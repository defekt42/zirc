/*
* Secure IRC Client (Libevent + TLS) - ZIRC-SEC v1.7
*
...
* 3. Protocol integrity enforcement (CR/LF injection prevention)
* 4. Memory zeroization for sensitive data
* 5. TLS 1.2+ with certificate verification
* 6. Rate limiting (5 msg/sec)
* 7. Re-entrant cleanup guards
* 8. Reconnection race condition prevention
* 9. Comprehensive error handling
* 10. NON-BLOCKING cleanup (Libevent timer)
* 11. STRICT ANSI ESCAPE STRIPPING in server output (FIXED)
*
* Compile on OpenBSD:
...
#define CONNECTION_TIMEOUT_SEC 40
...
/* CRITICAL FIX: Clean up existing connection before scheduling reconnect */
if (bev) {
...
/* A+ Overflow-safe exponential backoff */
...
/* Continue anyway with potentially stale timestamp */
} else {
...
/* --- A+ Timestamp Printer with Secure ANSI Stripping (FIXED) --- */
static void print_ts(const char *prefix, const char *msg) {
...
/* Color code processing with CRITICAL FIXES for bounds checking */
...
/* CRITICAL FIX: Bounds checking for all pointer increments */
if (*(p+1) && isdigit(*(p+1))) {
p++;
int fg = *p - '0';
...
/* Background color - CRITICAL: Check both p+1 and p+2 exist */
...
case 0x04: {
printf(ANSI_RESET);
/* Skip 6 hex characters with bounds checking */
...
case '\x1b': {
/* CRITICAL FIX: Properly consume ANSI escape sequences */
...
/* Numeric/Command Parser with CRITICAL FIX: NULL check */
...
size_t msg_overhead = 8 + strlen(CHANNEL) + 2 + 2; /* "PRIVMSG " + channel + " :" + "\r\n" */
...
printf(ANSI_BOLD ANSI_BRIGHT_GREEN
" ✓ OpenBSD Sandboxing Active (pledge: stdio inet rpath tty proc)\n"
ANSI_RESET);
} else {
...
"═══════════════════════════════════════════════════════════\n"
ANSI_RESET);
printf(ANSI_BOLD " ZIRC-SEC v1.7 (Zeroization Security Applied) \n" ANSI_RESET);
printf(ANSI_BOLD ANSI_BRIGHT_YELLOW
"═══════════════════════════════════════════════════════════\n"
ANSI_RESET);
printf("\n");
...
printf(ANSI_BOLD ANSI_BRIGHT_GREEN " ✓ Message Rate Limiting (5 msg/sec)\n" ANSI_RESET);
