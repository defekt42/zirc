/*
* Secure IRC Client (Libevent + TLS) - FINAL VERSION with CORRECTED 256-COLOR MAPPING
*
* This version incorporates all previous fixes (linker, auto-join, security)
* and includes the customized irc_to_256 color map to correct the visual
* appearance of IRC colors in the terminal.
*
* Compile (adjust -I/-L paths as needed):
* gcc -o circ-sec circ-sec.c \
...
*
* Usage:
* ./circ-sec <server> <port> <nick> [nickserv_pass or 'prompt']
*/
...
/*
* This array maps the 16 standard IRC color codes (0-15) to specific indices
* in the 256-color ANSI palette, ensuring the colors are visually "correct"
* (i.e., match the traditional IRC client aesthetic).
*/
...
static int is_valid_char(unsigned char c) {
    ...
    if (c == 0x02 || c == 0x03 || c == 0x0F || c == 0x16 || c == 0x1D || c == 0x1F || c == 0x04 || c == 0x01) return 1;
    return 0;
}
            ...
            case 0x03:
                ...
                if (*p && isdigit(*p)) {
                    ...
                    if (fg_color_id >= 0 && fg_color_id < 16) {
                        // Use the custom mapping for standard IRC colors
                        printf("\x1b[38;5;%dm", irc_to_256[fg_color_id]);
                    } else if (fg_color_id >= 16 && fg_color_id < 256) {
                        // Passthrough for extended 256 colors
                        printf("\x1b[38;5;%dm", fg_color_id);
                    } else {
                        // Handle out-of-range color by resetting
                        printf(ANSI_RESET);
                    }
                    // --- END OF COLOR FIX ---
...
int main(int argc, char **argv) {
    ...
    printf(ANSI_BOLD "Zeroization Secure IRC Client v1.0 (TLS 1.2+ Required) \n" ANSI_RESET);
    printf(ANSI_BOLD ANSI_BRIGHT_YELLOW"═══════════════════════════════════════════════════════════\n" ANSI_RESET);
    printf("\n");
    ...
    printf(" ✓ Strong Cipher Suites (PFS)\n");
    printf(" ✓ Password Zeroization\n");
    printf(" ✓ Connection Timeouts\n");
    printf(" ✓ Auto-Reconnect with Backoff\n");
    printf(" ✓ Input Validation\n");
    printf(" ✓ Delayed JOIN (Cloak Safety)\n");
    printf(" ✓ Robust Hostmask Hiding\n");
    printf(" ✓ CORRECTED 256-Color Support\n");
    printf(" ✓ Log File Rotation (Max %ld MB)\n", (long)LOG_MAX_SIZE / 1024 / 1024);
    printf("\n");
    ...
    printf(" " ANSI_BOLD "/JOIN #channel" ANSI_RESET " - Join a channel\n");
    printf(" " ANSI_BOLD "/MSG nick msg" ANSI_RESET " - Private message\n");
