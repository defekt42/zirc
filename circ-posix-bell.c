...
- Features: Non-blocking I/O, dynamic keep-alive, color support, logging, and VISUAL BELL
-
- Compile:
- gcc -o irc irc_client.c -lssl -lcrypto -O2 -Wall
-
- Usage:
- ./irc <server> <port> <nick> [nickserv_pass]
...
#define ANSI_MAGENTA "\x1b[35m" // Purple/Magenta for client's own nick
#define ANSI_RED "\x1b[31m" // Red for error warnings
...
static void print_ts(FILE *f, const char *prefix, const char *msg) {
    ...
    if (f) {
        /* Log format includes the full prefix (including ANSI codes) for simplicity */
        fprintf(f, "[%s] %s%s\n", timebuf, prefix, msg); 
        fflush(f);
    }
...
*/
static void handle_server_msg(char *line) {
    /* The main loop ensures the line now ends only with '\n' or nothing.
       We remove a potential final '\n' here for tokenization. */
    ...
    /* VISUAL BELL: Emit ASCII bell character for any non-PING server message */
    putchar('\a');

    /* Tokenization for command parsing and registration checks.
       Note: We use a copy for strtok to avoid modifying the original 'line' 
       which we need intact for robust message content extraction later. */
    char *words[10];
    int nwords = 0;
    ...
    if (nwords >= 4 && strcmp(words[1], "PRIVMSG") == 0) {
        ...
        // --- REVISED MESSAGE CONTENT EXTRACTION ---
        // For PRIVMSG: :prefix PRIVMSG target :message content
        // We find the start of the message by skipping the first 3 spaces 
        // in the original line, as this reliably lands us at the message-leading colon.
        
        char *msg_ptr = line; 
        int space_count = 0;

        // Skip the first 3 tokens/spaces: :prefix<space>PRIVMSG<space>target<space>
        for (int i = 0; line[i] != '\0'; i++) {
            if (line[i] == ' ') {
                space_count++;
                if (space_count == 3) {
                    // msg_ptr is now the character *after* the third space
                    msg_ptr = line + i + 1; 
                    break;
                }
            }
        ...
        // Check if the part after the third space starts with a colon (the message delimiter)
        if (msg_ptr != line && msg_ptr[0] == ':') {
            // Message content starts right after the colon
            message_content = msg_ptr + 1; 
        } else {
            // Fallback for unexpected formats (e.g. if the message content is actually empty)
        ...
        if (is_private_msg) {
            /* DM: Prefix is highlighted (Orange), message is wrapped in IRC Orange Color (Code 7) */
            // Console prefix: Bold, Underlined, Bright Yellow/Orange tag
            snprintf(prefix_buf, sizeof(prefix_buf), ANSI_BOLD ANSI_UNDER ANSI_BRIGHT_YELLOW 
            ...
            /* Standard Channel Message: Use nickname only, no hostmask. */
            // Format for standard channel PRIVMSG: [TARGET] <nick>: MESSAGE
            snprintf(prefix_buf, sizeof(prefix_buf), "[%s] <%s>: ", 
                     target, nickname); 
            display_msg = message_content;
        }
...
static void handle_user_input(char *line) {
    ...
    if (line[0] == '/') {
        ...
        /* New: /ME command (CTCP ACTION) */
            ...
            // 2. Local Echo (Consistency with incoming actions)
            char echo_prefix[256];
            // Format: [TARGET] * <colored nick>
            snprintf(echo_prefix, sizeof(echo_prefix), "[%s] * %s%s%s ",
                     CHANNEL, ANSI_MAGENTA, nick, ANSI_RESET);
                     
            // The 'msg' pointer already holds the action content
            print_ts(logfp, echo_prefix, msg); 
        ...
        // Manually format the local echo of the sent message with colored nick
        char echo_prefix[256];
        snprintf(echo_prefix, sizeof(echo_prefix), "[%s] <%s%s%s>: ",
                 CHANNEL, ANSI_MAGENTA, nick, ANSI_RESET);
        print_ts(logfp, echo_prefix, line);
    }
}
    ...
    while (1) {
        ...
        if (fds[0].revents & POLLIN) {
            ...
            if (n > 0) {
                ...
                /* *** BUFFER CANONICALIZATION (and safety trimming) *** */
                // Replace all \r\n and lone \r with a single \n
                char *src = server_buf;
                char *dst = server_buf;
                while (*src) {
                    if (*src == '\r') {
                ...
                *dst = '\0';
                server_buflen = (int)(dst - server_buf);
                /* ************************************************* */

                /* Process complete lines (now reliably split by '\n') */
                char *line_start = server_buf;
                char *line_end;
                while ((line_end = strchr(line_start, '\n')) != NULL) {
                    *line_end = '\0';
                    handle_server_msg(line_start);
                    line_start = line_end + 1; // Move past the single '\n'
                }
                ...
                if (server_buflen > 0) {
                    ...
                    server_buf[0] = '\0'; // Ensure clean buffer if nothing remains
                }
            } else if (n == 0) {
