...
#define CHANNEL "#test" // Default channel for sending PRIVMSG if no command is used
...
static void handle_server_msg(char *line) {
    ...
    if (nwords >= 4 && strcmp(words[1], "PRIVMSG") == 0) {
        ...
        // --- NEW RESILIENCE CHECK FOR JAMMED MESSAGES ---
        // If the message content looks like the start of a new IRC protocol message,
        // we stop formatting and fall through to the raw print.

        // 1. Check for new IRC prefix (starts with ':')
        if (message_content[0] == ':') {
            goto fall_through;
        }

        // 2. Check for numeric protocol data start (4 or more consecutive digits)
        // No valid IRC numeric code has more than 3 digits. 
        // Seeing 4+ consecutive digits (like '7004') means it's jammed data.
        if (isdigit(message_content[0]) && isdigit(message_content[1]) && 
            isdigit(message_content[2]) && isdigit(message_content[3])) {
            goto fall_through;
        }

        // 3. Check for raw command words that should never appear in a message body
        if (strncmp(message_content, "JOIN ", 5) == 0 || 
            strncmp(message_content, "MODE ", 5) == 0 ||
            strncmp(message_content, "NICK ", 5) == 0 ||
            strncmp(message_content, "QUIT ", 5) == 0 ||
            strncmp(message_content, "TOPIC ", 6) == 0 ||
            strncmp(message_content, "PRIVMSG ", 8) == 0) {
            goto fall_through;
        }
        // --------------------------------------------------
